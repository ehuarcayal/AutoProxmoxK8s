...
resource "proxmox_vm_qemu" "k8s_master" {
  ...
  sshkeys = file("${var.masters["ssh_key"]}")
}
...
resource "proxmox_vm_qemu" "k8s_worker" {
  ...
  sshkeys = file("${var.workers["ssh_key"]}")
...
data "template_file" "k8s" {
  ...
  vars = {
    k8s_master_ip = "${join("\n", [for instance in proxmox_vm_qemu.k8s_master : join("", [instance.default_ipv4_address, " ansible_ssh_private_key_file=", var.masters["ssh_key_ansible"], " ansible_user=", var.masters["user_host"]])])}"
    k8s_node_ip   = "${join("\n", [for instance in proxmox_vm_qemu.k8s_worker : join("", [instance.default_ipv4_address, " ansible_ssh_private_key_file=", var.workers["ssh_key_ansible"], " ansible_user=", var.workers["user_host"]])])}"
  }
}
